...
import type {
  ...
  KnockoutStageRounds,
  KnockoutMatch,
  TournamentState as FullTournamentState,
  Partner,
  TournamentMode,
  MatchComment,
  SeasonHistory,
...
const hydrateTournamentData = (
  ...
  const hydratedSettings =
    state.scheduleSettings || createInitialState(state.mode).scheduleSettings;
  ...
  return {
    ...
    knockoutStage: state.knockoutStage
      ? {
          "Play-offs": (state.knockoutStage["Play-offs"] || []).map(
            (m: any) => ({
              ...m,
              teamA: m.teamA ? getFullTeam(m.teamA) : null,
              teamB: m.teamB ? getFullTeam(m.teamB) : null,
            }),
          ),
          "Round of 16": (state.knockoutStage["Round of 16"] || []).map(
            (m: any) => ({
              ...m,
              teamA: m.teamA ? getFullTeam(m.teamA) : null,
              teamB: m.teamB ? getFullTeam(m.teamB) : null,
          ...
          "Quarter-finals": (state.knockoutStage["Quarter-finals"] || []).map(
            (m: any) => ({
              ...m,
              teamA: m.teamA ? getFullTeam(m.teamA) : null,
              teamB: m.teamB ? getFullTeam(m.teamB) : null,
          ...
          "Semi-finals": (state.knockoutStage["Semi-finals"] || []).map(
            (m: any) => ({
              ...m,
              teamA: m.teamA ? getFullTeam(m.teamA) : null,
              teamB: m.teamB ? getFullTeam(m.teamB) : null,
          ...
          Final: (state.knockoutStage["Final"] || []).map((m: any) => ({
            ...m,
            teamA: m.teamA ? getFullTeam(m.teamA) : null,
            teamB: m.teamB ? getFullTeam(m.teamB) : null,
          })),
...
type Action =
  ...
  | {
      type: "ARCHIVE_SEASON";
      payload: { historyEntry: SeasonHistory; keepTeams: boolean };
    }
  ...
  | {
      type: "DELETE_KNOCKOUT_MATCH";
      payload: { round: keyof KnockoutStageRounds; id: string };
    }
  ...
  | {
      type: "ADD_MATCH_COMMENT";
      payload: { matchId: string; comment: MatchComment };
    }
...
const tournamentReducer = (
  ...
  const mergeCommentsIntoMatches = (
    matches: Match[],
    commentsMap: Record<string, MatchComment[]>,
  ) => {
  ...
  const mergeCommentsIntoKnockout = (
    stage: KnockoutStageRounds | null,
    commentsMap: Record<string, MatchComment[]>,
  ) => {
    if (!stage) return null;
    const newStage = { ...stage };
    (Object.keys(newStage) as Array<keyof KnockoutStageRounds>).forEach(
      (key) => {
        newStage[key] = newStage[key].map((m) => ({
          ...m,
          comments: commentsMap[m.id] || [],
        }));
      },
  ...
  switch (action.type) {
    case "SET_STATE": {
      ...
      const matchesWithComments = mergeCommentsIntoMatches(
        rawState.matches || [],
        currentComments,
      );
      const knockoutWithComments = mergeCommentsIntoKnockout(
        rawState.knockoutStage,
        currentComments,
      );
      newState = {
        ...state,
        ...
        knockoutStage: knockoutWithComments,
        matchComments: currentComments,
      };
      break;
    }
    case "SET_COMMENTS": {
      const commentsMap = action.payload;
      const updatedMatches = mergeCommentsIntoMatches(
        state.matches,
        commentsMap,
      );
      const updatedKnockout = mergeCommentsIntoKnockout(
        state.knockoutStage,
        commentsMap,
      );
      newState = {
        ...state,
        matchComments: commentsMap,
        matches: updatedMatches,
        knockoutStage: updatedKnockout,
      };
      break;
    }
    case "SET_MODE":
      ...
      break;
    case "SET_SYSTEM":
      newState = { ...state, system: action.payload };
      break;
    case "SET_CUSTOM_NAME":
      newState = { ...state, customName: action.payload };
      break;
    case "ADD_TEAM":
    ...
    case "UPDATE_MATCH_SCORE": {
      const { matchId, scoreA, scoreB, proofUrl, playerStats, isWO, woTeamId } =
        action.payload;
      ...
      if (isWO) {
        ...
        updatedTeams = updatedTeams.map((t) => {
          if (
            woTeamId === "both" &&
            (t.id === match.teamA.id || t.id === match.teamB.id)
          ) {
      ...
      newState = {
        ...
        matches: state.matches.map((m) =>
          m.id === matchId
            ? { ...m, ...action.payload, status: "finished" as const }
            : m,
        ),
      };
      break;
    ...
    case "UPDATE_KNOCKOUT_MATCH": {
      ...
      ks[action.payload.round] = ks[action.payload.round].map((m) =>
    ...
    case "DELETE_KNOCKOUT_MATCH": {
      if (!state.knockoutStage) return state;
      const ks = { ...state.knockoutStage };
      ks[action.payload.round] = (ks[action.payload.round] || []).filter(
    ...
    case "GENERATE_KNOCKOUT_BRACKET":
      newState = { ...state, knockoutStage: action.payload };
      break;
    case "UPDATE_NEWS":
    ...
    case "UPDATE_NEWS_CATEGORIES":
      newState = { ...state, newsCategories: action.payload };
      break;
    case "UPDATE_SHOP_CATEGORIES":
      newState = { ...state, shopCategories: action.payload };
      break;
    case "UPDATE_MARQUEE":
      newState = { ...state, marqueeMessages: action.payload };
      break;
    case "UPDATE_RULES":
      newState = { ...state, rules: action.payload };
      break;
    case "UPDATE_BANNERS":
    ...
    case "UPDATE_HEADER_LOGO":
      newState = { ...state, headerLogoUrl: action.payload };
      break;
    case "UPDATE_PWA_ICON":
      newState = { ...state, pwaIconUrl: action.payload };
      break;
    case "SET_REGISTRATION_STATUS":
      newState = { ...state, isRegistrationOpen: action.payload };
      break;
    case "SET_STATUS":
      newState = { ...state, status: action.payload };
      break;
    case "ADD_HISTORY_ENTRY":
      newState = { ...state, history: [action.payload, ...state.history] };
      break;
    case "DELETE_HISTORY_ENTRY":
      newState = {
        ...state,
        history: state.history.filter((h) => h.seasonId !== action.payload),
      };
      break;
    case "UPDATE_VISIBLE_MODES":
      newState = { ...state, visibleModes: action.payload };
      break;
    case "UPDATE_HIDDEN_VIEWS":
      newState = { ...state, hiddenViews: action.payload };
      break;
    case "ADD_MATCH_COMMENT": {
      ...
      const newMatches = mergeCommentsIntoMatches(
        state.matches,
        newCommentsMap,
      );
      const newKnockout = mergeCommentsIntoKnockout(
        state.knockoutStage,
        newCommentsMap,
      );
      newState = {
        ...state,
        matches: newMatches,
        knockoutStage: newKnockout,
        matchComments: newCommentsMap,
      };
      break;
    }
    case "UPDATE_SCHEDULE_SETTINGS":
      newState = {
        ...state,
        scheduleSettings: { ...state.scheduleSettings, ...action.payload },
      };
      break;
    case "UPDATE_WO_PENALTY":
      newState = { ...state, woPenalty: action.payload };
      break;
    case "SET_ALL_TEAMS_SALDO":
      newState = {
        ...state,
        teams: state.teams.map((t) => ({ ...t, saldo: action.payload })),
      };
      break;
    ...
    case "RESET": {
      const initialState = createInitialState(state.mode);
      newState = {
        ...initialState,
...
export const useTournament = (activeMode: TournamentMode, isAdmin: boolean) => {
  ...
  useEffect(() => {
    setIsLoading(true);
    const unsubscribeTournament = subscribeToTournamentData(
      activeMode,
      (serverData) => {
    ...
    const unsubscribeComments = subscribeToMatchComments(
      activeMode,
      (commentsMap) => {
        isUpdatingFromServer.current = true;
  ...
  const setResetCycle = (cycle: 24 | 48) =>
    dispatch({
      type: "UPDATE_SCHEDULE_SETTINGS",
      payload: { resetCycleHours: cycle, lastResetTime: Date.now() },
    });

  const resolveTeamClaim = useCallback(
    async (teamId: string, approved: boolean) => {
      if (!isAdmin) {
  ...
  const generateKnockoutBracket = useCallback(() => {
    const topTeamsByGroup: { winner: Team | null; runnerUp: Team | null }[] =
    ...
    if (topTeamsByGroup.some((g) => !g.winner))
      return {
        success: false,
        message: "Pastikan semua grup memiliki pemenang.",
      };
    ...
    if (state.groups.length === 2) {
      ...
      newKnockout["Semi-finals"].push({
        ...
        scoreB2: null,
        winnerId: null,
        nextMatchId: "ko-final-1",
        placeholderA: "Winner A",
        placeholderB: "Runner B",
      });
      newKnockout["Semi-finals"].push({
        id: `ko-sf-2`,
        round: "Semi-finals",
        ...
        winnerId: null,
        nextMatchId: "ko-final-1",
        placeholderA: "Winner B",
        placeholderB: "Runner A",
      });
      ...
      if (topTeams.length < 2)
        return { success: false, message: "Minimal butuh 2 tim." };
      const roundName =
        topTeams.length <= 4
          ? "Semi-finals"
          : topTeams.length <= 8
  ...
  const autoGenerateGroups = useCallback(
    (numberOfGroups: number) => {
      ...
      dispatch({
        type: "GENERATE_GROUPS",
        payload: { groups: newGroups, matches: [], knockoutStage: null },
      });
    },
    [state.teams],
  );
  ...
  const clubStats = useMemo(() => {
    ...
    state.teams.forEach((t) => {
      if (t && t.id) stats[t.id] = { team: t, goals: 0 };
    });
    const processMatch = (m: Match | KnockoutMatch) => {
      if (!m) return;
      if ("status" in m) {
        if (m.status !== "finished") return;
        if (m.teamA?.id && stats[m.teamA.id])
          stats[m.teamA.id].goals += m.scoreA || 0;
        if (m.teamB?.id && stats[m.teamB.id])
          stats[m.teamB.id].goals += m.scoreB || 0;
      } else {
        if (m.scoreA1 === null) return;
        if (m.teamA?.id && stats[m.teamA.id])
          stats[m.teamA.id].goals += (m.scoreA1 || 0) + (m.scoreA2 || 0);
        if (m.teamB?.id && stats[m.teamB.id])
          stats[m.teamB.id].goals += (m.scoreB1 || 0) + (m.scoreB2 || 0);
      }
    };
    state.matches.forEach(processMatch);
    if (state.knockoutStage) {
      (Object.values(state.knockoutStage).flat() as KnockoutMatch[]).forEach(
        (m) => {
          if (m) processMatch(m);
        },
      );
  ...
  return {
    ...
    isSyncing,
    clubStats,
    playerStats: { topScorers: [], topAssists: [] },
    hasUnsavedChanges,
    forceSave,
    ...
    updateNewsCategories: (cats: string[]) =>
      dispatch({ type: "UPDATE_NEWS_CATEGORIES", payload: cats }),
    updateShopCategories: (cats: string[]) =>
      dispatch({ type: "UPDATE_SHOP_CATEGORIES", payload: cats }),
    updateMarqueeMessages: (msgs: string[]) =>
      dispatch({ type: "UPDATE_MARQUEE", payload: msgs }),
    addTeam: (
      id: string,
      name: string,
      logoUrl: string,
    ...
    updateKnockoutMatch: (id: string, m: KnockoutMatch) =>
      dispatch({ type: "UPDATE_KNOCKOUT_MATCH", payload: m }),
    addKnockoutMatch: (
    ...
    deleteKnockoutMatch: (round: keyof KnockoutStageRounds, id: string) =>
      dispatch({ type: "DELETE_KNOCKOUT_MATCH", payload: { round, id } }),
    generateKnockoutBracket,
    initializeEmptyKnockoutStage: () =>
      dispatch({
        type: "GENERATE_KNOCKOUT_BRACKET",
        payload: {
          "Play-offs": [],
          "Round of 16": [],
          "Quarter-finals": [],
    ...
    updateBanners: (b: string[]) =>
      dispatch({ type: "UPDATE_BANNERS", payload: b }),
    updatePartners: (p: Partner[]) =>
      dispatch({ type: "UPDATE_PARTNERS", payload: p }),
    updateHeaderLogo: (url: string) =>
      dispatch({ type: "UPDATE_HEADER_LOGO", payload: url }),
    updatePwaIcon: (url: string) =>
      dispatch({ type: "UPDATE_PWA_ICON", payload: url }),
    setRegistrationOpen: (open: boolean) =>
      dispatch({ type: "SET_REGISTRATION_STATUS", payload: open }),
    setTournamentStatus: (status: "active" | "completed") =>
      dispatch({ type: "SET_STATUS", payload: status }),
    addHistoryEntry: (entry: SeasonHistory) =>
      dispatch({ type: "ADD_HISTORY_ENTRY", payload: entry }),
    deleteHistoryEntry: (id: string) =>
      dispatch({ type: "DELETE_HISTORY_ENTRY", payload: id }),
    setTournamentState: (s: FullTournamentState) =>
      dispatch({ type: "SET_STATE", payload: s }),
    updateVisibleModes: (modes: TournamentMode[]) =>
      dispatch({ type: "UPDATE_VISIBLE_MODES", payload: modes }),
    updateHiddenViews: (views: View[]) =>
      dispatch({ type: "UPDATE_HIDDEN_VIEWS", payload: views }),
    setTournamentSystem: (system: TournamentSystem) =>
      dispatch({ type: "SET_SYSTEM", payload: system }),
    setCustomName: (name: string) =>
      dispatch({ type: "SET_CUSTOM_NAME", payload: name }),
    resetTournament: () => {
      clearGlobalChat();
      dispatch({ type: "RESET", payload: createInitialState(state.mode) });
    },
    archiveSeason: (historyEntry: SeasonHistory, keepTeams: boolean) =>
      dispatch({
        type: "ARCHIVE_SEASON",
        payload: { historyEntry, keepTeams },
      }),
    manualAddGroup: (n: string) =>
      dispatch({
        type: "GENERATE_GROUPS",
        payload: {
          groups: [
            ...state.groups,
            { id: `g${Date.now()}`, name: n, teams: [], standings: [] },
          ],
          matches: state.matches,
          knockoutStage: state.knockoutStage,
        },
      }),
    manualDeleteGroup: (id: string) =>
      dispatch({
        type: "GENERATE_GROUPS",
        payload: {
    ...
    manualRemoveTeamFromGroup: (tid: string, gid: string) => {
      const newGroups = state.groups.map((g) =>
        g.id === gid ? { ...g, teams: g.teams.filter((x) => x.id !== tid) } : g,
      );
      dispatch({
        type: "GENERATE_GROUPS",
        payload: {
    ...
    updateWoPenalty: (p: number) =>
      dispatch({ type: "UPDATE_WO_PENALTY", payload: p }),
    setAllTeamsSaldo: (amount: number) =>
      dispatch({ type: "SET_ALL_TEAMS_SALDO", payload: amount }),
    updatePreviousRanks: () => dispatch({ type: "UPDATE_PREVIOUS_RANKS" }),
    generateMatchesFromGroups: (
      roundRobinType: "single" | "double" = "single",
    ) => {
      const newMatches: Match[] = [];
      state.groups.forEach((g) => {
        ...
        if (roundRobinType === "double") {
          const leg1Matches = newMatches.filter(
            (m) => m.group === g.id && m.leg === 1,
          );
          leg1Matches.forEach((m) => {
            newMatches.push({
              ...
              group: g.id,
              leg: 2,
              matchday: (m.matchday || 0) + roundsPerLeg,
            });
          });
        }
      });
