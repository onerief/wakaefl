...
export const deepClean = (input: any, stack = new WeakSet()): any => {
  ...
  if (
    ...
    (typeof (input as any).preventDefault === "function" &&
      typeof (input as any).stopPropagation === "function")
  ) {
    return undefined;
  }
...
export const getTournamentData = async (
  ...
  try {
    ...
    const combinedHistory =
      globalData.history && globalData.history.length > 0
        ? globalData.history
        : modeData.history || [];
    ...
    return {
      ...
      visibleModes: globalData.visibleModes || [
        "league",
        "wakacl",
        "two_leagues",
        "custom",
...
export const saveTournamentData = async (
  ...
  try {
    ...
    const cleanKnockout = state.knockoutStage
      ? sanitizeData(state.knockoutStage)
      : null;
...
export const subscribeToTournamentData = (
  ...
  if (!firestore || !mode) {
    if (errorCallback)
      errorCallback("Firestore not initialized or mode missing");
    return () => {};
  }
  ...
  const emit = () => {
    if (modeData !== undefined && globalData !== undefined) {
      ...
      const combinedHistory =
        safeGlobal.history && safeGlobal.history.length > 0
          ? safeGlobal.history
          : safeMode.history || [];
      ...
      callback({
        ...
        history: combinedHistory,
        visibleModes: safeGlobal.visibleModes || [
          "league",
          "wakacl",
          "two_leagues",
          "custom",
...
export const updateMatchProof = async (
  mode: TournamentMode,
  matchId: string,
  proofUrl: string,
) => {
...
export const getGlobalStats = async () => {
  ...
  try {
    ...
    const settingsSnap = await getDoc(
      doc(firestore, TOURNAMENT_COLLECTION, SETTINGS_DOC),
    );
    const partnerCount = settingsSnap.exists()
      ? (settingsSnap.data().partners || []).length
      : 0;
...
export const addMatchCommentToFirestore = async (
  mode: TournamentMode,
  matchId: string,
  ...
  try {
    ...
    if (
      match.teamA.ownerEmail &&
      match.teamA.ownerEmail.toLowerCase() !== comment.userEmail.toLowerCase()
    ) {
      ownersToNotify.add(match.teamA.ownerEmail);
    }
    if (
      match.teamB.ownerEmail &&
      match.teamB.ownerEmail.toLowerCase() !== comment.userEmail.toLowerCase()
    ) {
...
export const subscribeToGlobalNotifications = (
  ...
  return onSnapshot(q, (snap) => {
    ...
    snap.forEach((docSnap) =>
      notifs.push({ ...docSnap.data(), id: docSnap.id } as Notification),
    );
    callback(notifs);
  });
};
...
export const subscribeToMatchComments = (
  mode: TournamentMode,
  callback: (comments: Record<string, MatchComment[]>) => void,
) => {
    ...
    return onSnapshot(
      ...
      (err: any) => {
        if (err.message && err.message.includes("index")) {
          console.error(
            "PENTING: KLIK LINK INI UNTUK AKTIFKAN FITUR CHAT:",
            err.message,
          );
        } else {
...
export const addApprovedTeamToFirestore = async (
  mode: TournamentMode,
  team: Team,
) => {
  if (!firestore || !mode) return;
...
export const submitTeamClaimRequest = async (
  mode: TournamentMode,
  teamId: string,
  email: string,
) => {
...
export const resolveTeamClaimInFirestore = async (
  mode: TournamentMode,
  teamId: string,
  approved: boolean,
) => {
  ...
  await runTransaction(firestore, async (transaction) => {
    ...
    transaction.set(newNotifDoc, {
      ...
      title: approved
        ? "Permintaan Klaim Diterima"
        : "Permintaan Klaim Ditolak",
      message: approved
...
export const updateUserTeamData = async (
  mode: TournamentMode,
  teamId: string,
...
export const clearGlobalChat = async () => {
  if (!firestore) return;
  try {
    const snap = await getDocs(collection(firestore, CHAT_COLLECTION));
...
export const subscribeToDonations = (
  ...
  return onSnapshot(q, (snap) => {
    ...
    snap.forEach((docSnap) =>
      donations.push({ ...docSnap.data(), id: docSnap.id } as Donation),
    );
    callback(donations);
  });
};
