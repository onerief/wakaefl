        ...
        <div className="flex flex-col gap-1 w-full relative">
            <Wrapper 
                className={`
                    ...
                    ${isWinner 
                        ? 'bg-brand-vibrant text-white scale-[1.02] sm:scale-[1.05] z-10 border-white/20 shadow-[0_0_20px_var(--brand-vibrant)] ring-2 ring-brand-vibrant' 
                        : isLoser 
                            ? 'bg-brand-secondary/40 grayscale opacity-40 text-brand-light border-brand-accent'
                            : isMyTeam 
                                ? 'bg-brand-vibrant/10 text-brand-text border-brand-special shadow-[0_0_15px_var(--brand-special)]'
                                : 'bg-brand-secondary/80 text-brand-text border-brand-accent hover:border-brand-light/30'
                    }
                    ${isOngoing && !isWinner ? 'border-brand-vibrant/20 bg-brand-secondary/80' : ''}
                `} 
                {...wrapperProps}
            >
                {team ? (
                    <>
                        ...
                        <div className="flex-grow min-w-0">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className={`text-[6px] sm:text-[7px] font-black px-1 rounded border ${isWinner ? 'border-white/20 bg-white/10' : 'border-brand-accent bg-brand-primary/20 text-brand-light/50'}`}>
                                    {isHome ? 'H' : 'A'}
                                </span>
                            </div>
                            {isWinner && (
                    ...
                    <div className="flex items-center gap-2 w-full opacity-20">
                        <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-brand-primary/10 flex items-center justify-center text-brand-light border border-brand-accent">
                            <Shield size={8} />
                        </div>
            ...
            {isAdminMode && team && onAdjust && (
                <div className="flex items-center justify-center gap-2 bg-brand-primary/80 rounded-lg p-0.5 mx-1 mt-1 border border-brand-accent scale-90">
...
const KnockoutMatchCard: React.FC<{ 
    ...
    return (
        <div className="relative py-2 sm:py-4 w-full">
            {!isFinal && (
                <div className="hidden lg:block absolute -right-16 top-1/2 -translate-y-1/2 w-16 h-px bg-brand-accent z-0">
                    <div className={`h-full bg-brand-vibrant transition-all duration-1000 shadow-[0_0_10px_var(--brand-vibrant)] ${anyWinner ? 'w-full opacity-100' : 'w-0 opacity-0'}`}></div>
                </div>
            )}
                ...
                <div className="flex flex-col items-center py-0.5">
                    {isFinished ? (
                        <div className="relative">
                            ...
                            >
                                <div className="flex items-center gap-2 sm:gap-3">
                                    <span className="text-sm sm:text-xl font-black italic text-brand-text leading-none">
                                        {isFinal ? `${sA1}-${sB1}` : `${aggA}-${aggB}`}
                                    </span>
                                    {!isFinal && (
                                        <div className="p-0.5 sm:p-1 bg-brand-secondary rounded-full text-brand-vibrant group-hover/agg:bg-brand-vibrant group-hover/agg:text-white transition-colors">
                            ...
                            {showBreakdown && !isFinal && (
                                <div className="absolute top-[120%] left-1/2 -translate-x-1/2 w-44 sm:w-52 bg-brand-secondary border border-brand-accent rounded-2xl p-3 sm:p-4 shadow-2xl z-[50] animate-in slide-in-from-top-2 duration-300">
                                    <div className="space-y-2 sm:space-y-3">
                                        <div className="flex justify-between items-center bg-brand-primary/40 p-1.5 sm:p-2 rounded-xl">
                                            <span className="text-[7px] sm:text-[9px] font-black text-brand-light uppercase">Leg 1 (A-H)</span>
                                            <span className="text-[10px] sm:text-xs font-black text-brand-text">{sA1}-{sB1}</span>
                                        </div>
                                        <div className="flex justify-between items-center bg-brand-primary/40 p-1.5 sm:p-2 rounded-xl">
                                            <span className="text-[7px] sm:text-[9px] font-black text-brand-light uppercase">Leg 2 (B-H)</span>
                                            <span className="text-[10px] sm:text-xs font-black text-brand-text">{sA2}-{sB2}</span>
                                        </div>
                                        <div className="h-px bg-brand-accent my-1"></div>
                                        <div className="flex justify-between items-center px-1">
                                            <span className="text-[8px] sm:text-[10px] font-black text-brand-vibrant uppercase">Agg</span>
                                            <span className="text-xs sm:text-sm font-black text-brand-text">{aggA}-{aggB}</span>
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="flex flex-col items-center gap-1">
                            <div className="text-[8px] sm:text-[10px] font-black text-white uppercase tracking-widest bg-brand-vibrant px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-full shadow-[0_0_15px_var(--brand-vibrant)] animate-pulse flex items-center gap-1.5">
                        ...
                        <div className="flex flex-col items-center">
                            <div className="w-px h-3 sm:h-6 bg-gradient-to-b from-brand-accent to-transparent"></div>
                            <span className="text-[7px] sm:text-[9px] font-black text-brand-light/20 uppercase tracking-[0.4em] py-0.5">vs</span>
                            <div className="w-px h-3 sm:h-6 bg-gradient-to-t from-brand-accent to-transparent"></div>
                        </div>
                    )}
                </div>
            ...
            {isAdminMode && (
                <div className="mt-4 sm:mt-6 p-2 sm:p-3 bg-brand-primary/80 rounded-2xl border border-brand-accent space-y-2 sm:space-y-3 relative z-20">
                    {!isFinal && (
                         <div className="flex items-center justify-between gap-2 px-1">
                            <div className="flex-1 flex flex-col items-center gap-0.5">
                                <span className="text-[6px] sm:text-[8px] font-black text-brand-light uppercase">L2: A</span>
                                <div className="flex items-center gap-1.5 sm:gap-2 bg-brand-secondary/60 p-1 rounded-xl border border-brand-accent">
...
export const KnockoutStageView: React.FC<KnockoutStageViewProps> = ({ knockoutStage, onSelectTeam, isAdminMode, onUpdateScore, userOwnedTeamIds = [] }) => {
  ...
  if (availableRounds.length === 0) return (
      <div className="py-20 text-center bg-brand-secondary/30 rounded-3xl border border-dashed border-brand-accent">
  ...
  return (
    <div className="w-full relative pb-10">
              ...
              <div className="relative inline-block group">
                  ...
                  <div className="relative bg-gradient-to-b from-brand-special/20 to-transparent border border-brand-special/30 p-8 rounded-[3rem] shadow-2xl">
                      <div className="flex flex-col items-center">
                          ...
                          <TeamLogo logoUrl={winnerTeam.logoUrl} teamName={winnerTeam.name} className="w-24 h-24 sm:w-32 sm:h-32 mb-4 ring-4 ring-brand-special/50 shadow-[0_0_30px_var(--brand-special)]" />
      ...
      <div className="flex flex-col gap-4 mb-8 px-2 animate-in slide-in-from-top duration-500">
          <div className="flex items-center justify-between bg-brand-secondary/80 border border-brand-accent rounded-2xl p-1.5 shadow-xl backdrop-blur-md max-w-2xl mx-auto w-full">
              <button 
                onClick={() => setActiveRoundIdx(prev => Math.max(0, prev - 1))}
                disabled={activeRoundIdx === 0}
                className="p-3 text-brand-light hover:text-brand-text disabled:opacity-20 transition-all active:scale-75"
              >
                  <ChevronLeft size={24} />
              </button>
              ...
              <div className="flex flex-col items-center">
                  ...
                  <h3 className="text-base sm:text-xl font-black text-brand-text uppercase italic tracking-tight text-center truncate px-2">
                      {activeRoundName}
                  </h3>
              </div>
      ...
      <div className="w-full overflow-hidden">
          <div className="flex flex-col lg:flex-row gap-6 sm:gap-12 lg:gap-32 lg:justify-center lg:min-w-full px-2 sm:px-10">
            ...
            <div className="absolute top-0 right-0 p-4 sm:p-10 flex flex-col items-end opacity-5 sm:opacity-10 pointer-events-none select-none overflow-hidden hidden lg:flex">
                <h2 className="text-4xl sm:text-8xl font-black text-brand-text text-right italic leading-none tracking-tighter">
            ...
            {activeRoundName === 'Final' && (
                <div className={`flex flex-col items-center justify-center pt-8 sm:pt-24 px-6 sm:px-12 w-full lg:w-auto animate-in zoom-in duration-700`}>
                    <div className="relative group">
                        ...
                        <div className="relative w-24 h-24 sm:w-40 sm:h-40 rounded-[1.5rem] sm:rounded-[2.5rem] bg-gradient-to-br from-brand-secondary via-brand-primary to-brand-primary border border-brand-special/40 flex items-center justify-center z-10 shadow-[0_0_50px_var(--brand-special)] hover:scale-110 transition-transform duration-700">
                            <div className="flex flex-col items-center">
                                <Trophy size={32} className="text-brand-special drop-shadow-[0_0_15px_var(--brand-special)] animate-float sm:w-[64px] sm:h-[64px]" />
                            </div>
                        </div>
      ...
      <div className="fixed bottom-24 lg:bottom-6 left-1/2 -translate-x-1/2 opacity-20 pointer-events-none flex items-center gap-2 lg:gap-4 z-0">
          ...
          <span className="text-xs lg:text-2xl font-black text-brand-text italic tracking-tighter uppercase whitespace-nowrap">WakaEFLeague Championship</span>
      </div>
    </div>
  );
};
