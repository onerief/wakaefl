
import React from 'react';
import { Card } from '../shared/Card';
import { Button } from '../shared/Button';
import { X, AlertTriangle, Trash2 } from 'lucide-react';

interface ResetConfirmationModalProps {
  onConfirm: () => void;
  onCancel: () => void;
}

export const ResetConfirmationModal: React.FC<ResetConfirmationModalProps> = ({ onConfirm, onCancel }) => {
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[150] backdrop-blur-md p-4 animate-in fade-in zoom-in-95 duration-200">
      <Card className="w-full max-w-xs relative !p-0 overflow-hidden shadow-2xl bg-brand-primary rounded-2xl border border-red-500/30">
        <div className="bg-gradient-to-r from-red-900/40 to-black p-3 border-b border-white/5 flex justify-between items-center">
            <h3 className="text-xs font-black text-white italic uppercase tracking-tighter flex items-center gap-2">
                <AlertTriangle size={14} className="text-red-500" /> Hard Reset
            </h3>
            <button onClick={onCancel} className="text-brand-light hover:text-white transition-all bg-white/5 p-1 rounded-full"><X size={12} /></button>
        </div>
        
        <div className="p-4">
            <div className="bg-red-500/10 p-3 rounded-xl border border-red-500/20 mb-4 flex gap-3">
                <Trash2 className="text-red-500 shrink-0 mt-0.5" size={16} />
                <div className="space-y-1">
                    <p className="text-[10px] font-black text-red-500 uppercase tracking-widest">Tindakan Irreversibel</p>
                    <p className="text-[10px] text-brand-light leading-relaxed">
                        Anda akan menghapus <strong>SEMUA KONFIGURASI DAN DATA</strong>: Tim, Jadwal, Skor, Banner, Berita, dan Pengaturan lainnya. 
                        <br/><br/>
                        Hanya <strong>Hall of Fame (History)</strong> yang akan tersimpan.
                    </p>
                </div>
            </div>
            
            <div className="flex justify-end gap-2">
                <Button type="button" variant="secondary" onClick={onCancel} className="!text-[10px] !py-1.5 rounded-lg h-8">
                    Batalkan
                </Button>
                <Button 
                    type="button" 
                    onClick={onConfirm}
                    className="bg-red-600 text-white hover:bg-red-700 !text-[10px] !py-1.5 rounded-lg shadow-lg shadow-red-900/20 font-black uppercase tracking-wider h-8"
                >
                    Ya, Reset Total
                </Button>
            </div>
        </div>
      </Card>
    </div>
  );
};
